...
DOCKER_IMAGE=$(grep 'FROM golang:' Dockerfile | head -n 1 | sed -E 's/.*FROM ([^ ]+).*/\1/')
echo "INFO: Inspecting Docker image: $DOCKER_IMAGE"

# Use skopeo to inspect the image and extract Go version from GOLANG_VERSION env variable
# Example env value: "GOLANG_VERSION=1.21.13"
DOCKER_GO_VERSION=$(skopeo inspect docker://$DOCKER_IMAGE | jq -r '.Env[] | select(startswith("GOLANG_VERSION="))' | cut -d= -f2 | sed -E 's/([0-9]+\.[0-9]+).*/\1/')
